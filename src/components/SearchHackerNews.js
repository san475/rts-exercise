...
const SearchHackerNews = () => {
    ...
    const queryHackerNews = (query) => {
        ...
        fetch(`${apiPath}?query=${query}`).then(res =>
            res.json().then(data => {
                if (data.hits)
    ...
    return <>
        <div className='search'>
            ...
            <button onClick={() => queryHackerNews(query)}>Search</button>
        </div>
    ...
    const createHitDisplay = (hit) => {
        return <div className='query-hit'>
            <h3> <a href={hit.url || hit.story_url} target='_blank' rel="noreferrer"> {hit.title || hit.story_title}</a>
            </h3> <div className='query-hit-header'>
